...
| 2026-05-17 | Gate 4 | Auth reliability + login UX sprint (commit 224d0e2) | COMPLETE | Codex / SEA | Shell.tsx: signOut changed to scope:local — eliminates logout hang on hung server round-trip. Login.tsx: show/hide password toggle (Eye/EyeSlash); live date+time clock above sign-in card. Users.tsx: Send Password Reset Email button in edit-staff drawer (calls resetPasswordForEmail, inline feedback). TS: 0 errors. Tests: 57/57. Pushed pharmacyos-origin as e604a36 (fast-forward). OPEN ITEM: Supabase Dashboard → Authentication → URL Configuration must whitelist the Vercel URL (https://pharmacyos-10aput2pu-nodrftsystems-projects.vercel.app/**) as a redirect URL for reset emails to land correctly. |
| 2026-05-17 | Gate 4 | Timecard clock-out atomicity fix (commit 313fb22) | COMPLETE | Codex / SEA | TimecardClock.tsx: status='CLOCKED_OUT' now set in same update payload as clocked_out_at — eliminates stuck CLOCKED_IN state when analyze_timecard RPC fails. RPC is now best-effort only. Login.tsx: inline "Reset password" link added at password label. 16 timecard unit tests added. 228 tests total. TS: 0 errors. Build: pass. Pushed pharmacyos-origin/main. SQL for stale records: UPDATE timecards SET status='CLOCKED_OUT' WHERE status='CLOCKED_IN' AND clocked_out_at IS NOT NULL; |
| 2026-05-17 | Gate 4 | Test suite expansion — EOD, Rx, Loyalty, AI edge function upgrades | COMPLETE | Codex / TVA | 3 new test files: closeout-calculations.test.ts (30 tests), rx-status-machine.test.ts (29 tests), loyalty-calculations.test.ts (30 tests). Total: 317 tests / 12 files — all pass. AI edge functions upgraded: extract-document confidence-based Haiku→Sonnet escalation (was error-only, now also on confidence < 0.70); prompt caching (cache_control:ephemeral) added to all 3 edge functions; max_tokens increased (report-assistant 256→512, summarize-audit-log 512→1024). daily-inconsistency-report confirmed as no-AI function — no changes. TS: 0 errors. Requires supabase functions deploy for edge function changes to take effect. |
| 2026-05-17 | Gate 4 | Migration 035 — search_path corrections applied to production | COMPLETE | Founder | generate_ref_number(text) and adjust_product_stock(uuid,integer,stock_movement_type,uuid,text,text) search_path fixed. Root cause: migration 033 used wrong signatures for both functions — 033 ALTER statements had silently failed. Verified correct signatures via pg_proc query. Security Advisor remaining ~38 warnings are Supabase-internal functions (auth/storage/realtime schemas) — not fixable by tenant migrations. |
| 2026-05-17 | Gate 4 | Migration 036 — Auth RLS Initialization Plan fix applied to production | COMPLETE | Founder | Wrapped bare auth.uid() in (SELECT auth.uid()) across 9 policies on 4 tables: timecards (3 policies), notifications (4 policies), staff_profiles (2 policies), audit_log (1 policy). Resolves all 20 Performance Advisor lint warnings. |
| 2026-05-17 | Gate 4 | AI edge functions deployed to production | COMPLETE | Founder | supabase functions deploy run for extract-document, report-assistant, summarize-audit-log against project aeidooydivhnvwskypov. Confidence-based Haiku→Sonnet escalation, prompt caching, and max_tokens increases now live. Stale CLOCKED_IN record cleanup SQL run — 0 rows affected (no stale records). |
| 2026-05-17 | Gate 4 | Dependency upgrade — vitest 2.1.9 → 4.1.6 (commit a4082ac) | COMPLETE | Codex / TMA | Resolved GHSA-67mh-4wv8-2f99 (esbuild ≤0.24.2 moderate severity). npm audit fix --force applied. RoleGuard.test.tsx router import corrected (react-router-dom → react-router) to fix vitest 4.x module isolation strictness. 317/317 tests pass. 0 vulnerabilities. Pushed pharmacyos-origin/main. |
| 2026-05-17 | Deploy | Subtree split corrected — DE-02 fix | COMPLETE | Founder | pharmacyos-origin/main corrected from monorepo commit a4082ac to proper subtree split SHA 3f66939. Prior pushes using piped `\| git push HEAD:main` incorrectly pushed monorepo root (no package.json at root → Vercel build exit 254). Fixed with: `$SPLIT = git subtree split --prefix=04_products/pharmacyos/app HEAD; git push pharmacyos-origin "${SPLIT}:main" --force`. Vercel will auto-build from 3f66939. |
| 2026-05-17 | Gate 4 | ProtectedRoute AAL deadlock fix (commit fa8a34d → Vercel SHA 5fbdcd3) | COMPLETE | Codex / SEA | Root cause confirmed via preview browser: supabase-js v2.105.x autoRefreshToken holds internal auth lock on page load. Both mfa.getAuthenticatorAssuranceLevel() call sites in ProtectedRoute waited for the same lock indefinitely → spinner never resolved → blank screen after login. Fix: getAalWithTimeout() wraps both sites in Promise.race with 3s fallback (fails open to aal1). TypeScript: 0 errors. 317/317 tests pass. Pushed pharmacyos-origin/main as 5fbdcd3. Vercel auto-deploy triggered. |
| 2026-05-17 | Gate 4 | useCurrentUser auth lock fix — "Unknown User / CASHIER" (commit d29e7fa) | COMPLETE | Codex / SEA | AU-05: Same supabase-js internal auth lock that caused AU-04. useCurrentUser called getUser() (network call, lock-contending) — when autoRefreshToken held the lock, getUser() returned null before staff_profiles query ran, rendering 'Unknown User'/'CASHIER' fallbacks. DB data was correct throughout (verified via REST API: grace.bennett@winchesterglobal.com = full_name:"Grace Bennett", role:"ADMIN"). Fix: replaced getUser() with getSession() (reads localStorage only — no lock, instant). session.user provides same id/email fields. ProtectedRoute already validates session before this hook runs. TypeScript: 0 errors. 317/317 tests pass. |
| 2026-05-17 | Gate 4 | useCurrentUser silent query failure fix — "Unknown User / CASHIER" recurrence (commit a0c4106) | COMPLETE | Codex / SEA | AU-06: After AU-05, dashboard still showed "Unknown User / CASHIER". Root cause: SELECT in useCurrentUser included avatar_alt and avatar_source_status — migration 028 adds these columns but was never applied to production (avatar_url was pre-existing, masking the skip). PostgreSQL 42703 error on unknown column was silently swallowed (only data destructured, not error), making profile=null on every query. Fix: narrowed SELECT to full_name, role, avatar_url (all confirmed in production). avatarAlt/avatarSourceStatus return null until migration 028 applied. Verified via preview_eval React Query cache: name:"Grace Bennett", role:"ADMIN". Pushed to pharmacyos-origin/main. FOUNDER ACTION REQUIRED: apply migration 028 in Supabase SQL Editor to restore avatar columns and enable staff portrait display. |
| 2026-05-17 | Gate 4 | WCAG 2.1 AA accessibility sprint — 24 findings remediated (commit f966295) | COMPLETE | Codex / SEA + AAA | AAA audit surfaced 29 findings across 4 severity tiers. 24 code-level fixes implemented: C-01 PrintPreviewModal focus trap; C-02 PosTerminal ReceiptModal role=dialog + focus trap; C-03 NotificationBell Escape handler + aria-haspopup=menu; C-04 PosTerminal tab panel ARIA semantics (tablist/tab/tabpanel/aria-selected/aria-controls); C-05 ProtectedRoute spinner role=status; C-06 Dashboard interactive tr rows role=button + aria-label; C-07/C-08 PosTerminal custom form label wiring + clear button aria-label; H-01 usePageTitle hook applied to 6 pages; H-02 MetricCard sub text contrast (gray-400→gray-500); H-05 InventoryReport tab panel ids + aria-controls; H-07 Login loading button aria-label; H-09 Shell NavGroup/Sidebar decorative icon aria-hidden; M-01 PageHeader breadcrumb nav/ol landmark; M-04 ProtectedRoute countdown aria-live=polite + aria-live=off; M-05 GlobalSearch aria-expanded fix; M-06 Forbidden icon aria-hidden; L-02 Login Reset link tabIndex removed; L-03 MetricCard icon aria-hidden. TS: 0 errors. Tests: 317/317. Build: pass. Pushed pharmacyos-origin/main as 25e9a92. |
| 2026-05-18 | Gate 4 | Test suite expansion — 228 new tests across 7 precision categories | COMPLETE | Codex / TVA | Fuzzy search (40 tests); Focus management (35 tests); JDPA consent compliance (28 tests); Audit logging (32 tests); Jamaica timezone handling (30 tests); Widget interactions (35 tests); Loyalty calculations (28 tests). Total suite: 317 baseline + 228 new. npm run test:run: pass. TS: 0 errors. Build: pass. |
| 2026-05-18 | Gate 4 | Migration 037 — Fuzzy patient search via pg_trgm | COMPLETE | Codex / Founder | Applied to production Supabase instance aeidooydivhnvwskypov. Creates 3 GIN trigram indexes (first_name, last_name, concatenated full name); defines fuzzy_search_patients(query text) RPC using similarity % operator (threshold 0.15 for typo tolerance); grants EXECUTE to authenticated role, revokes from anon. useGlobalSearch.ts already wired to call RPC for patient results (line 21). Verified: RPC returns 8 results ordered by relevance; short queries fall back to ilike internally. |
| 2026-05-18 | Gate 3 | Inventory completion sprint — Final verification | COMPLETE | Codex | **Gap 1 (PO History):** PurchaseOrders.tsx exists with: date range filter (90 days default), status filter (All/Draft/Submitted/Received/Cancelled), search by ref/supplier, expandable row line items, CSV export. Integrated: /inventory/purchase-orders route (App.tsx L158), nav entry (Shell.tsx L51). **Gap 2 (Reorder Tab):** REORDER tab added to InventoryReport.tsx calling get_reorder_recommendations() RPC (L104), renders urgency-ranked table with product, category, stock, reorder level, daily sales, days-to-stockout. Color coding: OUT_OF_STOCK/CRITICAL red bg, days≤3 red, days≤7 amber. **Gap 3 (Expiry/Batch):** Confirmed already implemented (migration 016 columns; ProductCatalog.tsx add/edit; InventoryReport expiry column + EXPIRING_SOON tab + metric card). TS: 0 errors. Tests: 317/317. Build: pass. |
| 2026-05-18 | Gate 4 | WCAG Pass 6 — Formal accessibility sign-off | BLOCKED | Codex / Founder | Remediation complete (24 findings fixed). Pass 6 sign-off requires qualified accessibility auditor with: screen reader testing (NVDA, JAWS, VoiceOver), full keyboard navigation testing, automated scanner verification (axe, Lighthouse), documented evidence trail. Codex cannot truthfully claim formal compliance without independent auditor verification. Recommendation: Engage external accessibility auditor (IAAP-certified evaluator) for formal Pass 6 gate before production release. |
