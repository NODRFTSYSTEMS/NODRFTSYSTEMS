...
| AU-05 | 2026-05-17 | **Dashboard shows "Unknown User / CASHIER"** for authenticated users even when `staff_profiles` record is correct | `useCurrentUser.ts` called `supabase.auth.getUser()` to get user identity. `getUser()` makes a network call AND contends for the same supabase-js internal auth lock held by `autoRefreshToken` on page load. When the lock is held, `getUser()` either returns null (causing early return) or waits long enough that React Query returns stale null data — so the `staff_profiles` query never executes and the fallback values `'Unknown User'` / `'CASHIER'` render. DB data (e.g. `full_name: "Grace Bennett"`, `role: "ADMIN"`) was correct throughout. | `useCurrentUser.ts`: replaced `supabase.auth.getUser()` with `supabase.auth.getSession()`. `getSession()` reads directly from localStorage — no network call, no lock contention, instant. `session.user` provides the same `id` and `email` fields needed for the `staff_profiles` query. Safe because `ProtectedRoute` already validates the session before this hook runs. Commit d29e7fa. 317/317 tests pass. | Preview browser: dashboard shows "Grace Bennett / ADMIN" immediately after login. | Recurrence — see AU-06 |
| AU-06 | 2026-05-17 | **Dashboard still shows "Unknown User / CASHIER"** — `staff_profiles` query failing silently on missing columns | `useCurrentUser.ts` SELECT included `avatar_alt` and `avatar_source_status`. Migration 028 adds these columns but was **never applied to production** (`avatar_url` was pre-existing from an earlier migration, masking the skip). Selecting non-existent columns causes PostgreSQL error `42703: column staff_profiles.avatar_alt does not exist` (HTTP 400). Error is silently swallowed — only `{ data: profile }` is destructured, not `{ error }` — so `profile = null` and both fallbacks trigger on every login. **Prevention rule: ALWAYS destructure AND log/handle the error from every Supabase query. Silent error swallowing = invisible production failures.** | `useCurrentUser.ts`: narrowed SELECT to `full_name, role, avatar_url` (confirmed to exist in production). `avatarAlt` and `avatarSourceStatus` hardcoded to `null` until migration 028 is applied. Code comment added referencing AU-06 and the restore path. Commit a0c4106. 317/317 tests pass. **Founder action required: apply migration 028 in Supabase SQL Editor to restore avatar_alt + avatar_source_status columns and enable staff portraits.** | Preview browser React Query cache: `name: "Grace Bennett"`, `role: "ADMIN"`. Screenshot confirmed. | — |
...
| R-07 | Running `tsc --noEmit` instead of `npm run build` before pushing | `tsc --noEmit` and Vercel's build use different TypeScript strictness and config resolution. Vercel runs `tsc -b && vite build` — this is stricter. **ALWAYS** run `npm run build` (inside `04_products/pharmacyos/app`) before any subtree split push. Zero errors from `tsc --noEmit` does NOT guarantee Vercel build success. |
| R-08 | Declaring a fix complete without verifying inside the exact runtime context | AU-05 was declared fixed after a manual `import('/src/lib/supabase.ts')` test in the preview browser — but that test did not reproduce the React Query + auth lock timing that the actual hook runs in. AU-06 was only found by reading the actual React Query cache state. **Always verify inside the running React app, not just in isolated eval calls.** |
| R-09 | Selecting DB columns without verifying they exist in production | `useCurrentUser.ts` selected `avatar_alt` and `avatar_source_status` — migration 028 adds them but was not applied to production. The query failed with code 42703, silently (error not destructured). **Before adding any column to a SELECT, verify it exists in production via: `SELECT column_name FROM information_schema.columns WHERE table_name = 'your_table'` or a direct REST call with the column name.** |
